...
datos<- readxl::read_excel("Graficas Percentiles.xlsx", sheet = "PercentilesITM", range = "C28:I42")
...
funcion_grafica <- function(pDatos, pVectorFechas){
  ...
  grafica <- ggplot(pDatos,
    ...
    geom_label(data = subset(pDatos, Cobertura == "Con coberturas FWD" & Mes <= as.Date('2024-08-01')),
               aes(x = xmin + (xmax - xmin)/2, y = `Mejor Escenario` +  0.22 * (`Mejor Escenario` - `Peor Escenario`),
                   label = scales::comma_format()(round(`Mejor Escenario`))),
               fill = "White", color = "black",
               vjust = 0.5, size = 3) +
    
    geom_label(data = subset(pDatos, Cobertura == "Con coberturas FWD" & Mes == as.Date('2024-09-01')),
               aes(x = xmin + (xmax - xmin)/2, y = `Mejor Escenario` -  0.18 * (`Mejor Escenario` - `Peor Escenario`),
                   label = scales::comma_format()(round(`Mejor Escenario`))),
               fill = "White", color = "black",
               vjust = 0.5, size = 3) +
    ...
    geom_label(data = subset(pDatos, Cobertura == "Con coberturas FWD" & Mes == as.Date('2024-06-01')),
               aes(x = xmin + (xmax - xmin)/2, y = `Peor Escenario` + 0.29 * (`Mejor Escenario` - `Peor Escenario`), 
                   label = scales::comma_format()(round(`Peor Escenario`))),
               fill = "White", color = "black",
               vjust = 0.5, size = 3) +
    
    geom_label(data = subset(pDatos, Cobertura == "Con coberturas FWD" & Mes == as.Date('2024-07-01')),
               aes(x = xmin + (xmax - xmin)/2, y = `Peor Escenario` - 0.1 * (`Mejor Escenario` - `Peor Escenario`), 
                   label = scales::comma_format()(round(`Peor Escenario`))),
               fill = "White", color = "black",
               vjust = 0.5, size = 3) +
...
graficaITM <- funcion_grafica(Ingresos,vector_fechas)

ggsave(plot = graficaITM,
       filename = "Utilidad Bruta ITM.png",
       width = 8, height = 5)
